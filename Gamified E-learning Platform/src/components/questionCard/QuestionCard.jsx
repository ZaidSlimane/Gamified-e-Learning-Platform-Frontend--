    ...
    return (
        <>
            {question && question.question_answer === 'no answer yet' ? (
                <div className={`shadow mt-3 question-card-container bg-white ${role !== 'TEACHER' ? '' : 'card-hover2'}`} onClick={role !== 'TEACHER' ? null : handleCardClick}>
                    {course && <p className="question-course-name mt-1">{course.courseName}</p>}
                    {chapter && <p className="question-chapter-name">Chapter : {chapter.chapterName}</p>}
                    <br />
                    {question && <p className="questio-question"><b>Question:</b> {question.question_content}</p>}
                    <br />
                </div>
            ) : (
